...
from ocv_model import OCVfromSOC_function  # previously converted core function
import os

def processOCP(data, cellID, minV, maxV, savePlots=False):
    """
    ...
    dataS1dismAh = np.array(data[k]['script1']['dismAh'])
    dataS1chgmAh = np.array(data[k]['script1']['chgmAh'])
    dataS1step = np.array(data[k]['script1']['step'])

    dataS3V = np.array(data[k]['script3']['voltage'])
    dataS3dismAh = np.array(data[k]['script3']['dismAh'])
    dataS3chgmAh = np.array(data[k]['script3']['chgmAh'])
    dataS3step = np.array(data[k]['script3']['step'])

    totDismAh = dataS1dismAh[-1] + dataS3dismAh[-1]
    totChgmAh = dataS1chgmAh[-1] + dataS3chgmAh[-1]
    eta25 = totDismAh / totChgmAh
    eta[k] = eta25

    # Adjust charge Ah for coulombic efficiency
    dataS1chgmAh = dataS1chgmAh * eta25
    dataS3chgmAh = dataS3chgmAh * eta25

    Q25 = dataS1dismAh[-1] - dataS1chgmAh[-1]
    Q[k] = Q25
    ...
    for k in not25:
        ...
        totDismAh = dataS1dismAh[-1] + dataS3dismAh[-1]
        totChgmAh = dataS1chgmAh[-1] + dataS3chgmAh[-1]
        eta[k] = totDismAh / totChgmAh

        dataS1chgmAh = dataS1chgmAh * eta[k]
        dataS3chgmAh = dataS3chgmAh * eta[k]

        Q[k] = dataS1dismAh[-1] - dataS1chgmAh[-1]
    ...
    for k in range(numtemps):
        ...
        ax.plot(SOC, raw_ocv, 'r', lw=1.2, label='Approximate OCP from data')
        ...
        ax.set_xlim([0, 1])
        ax.set_title(f'{cellID} OCP relationship at temp = {temp_k}°C')
    ...
    if savePlots:
        os.makedirs('OCP_FIGURES', exist_ok=True)
        plt.savefig(f'OCP_FIGURES/{cellID}_OCP_grid.png', dpi=300)
